...
class FinanceService:
    ...
    def get_dashboard_metrics():
        ...
        # Lent / Borrowed
        row_lent = conn.execute("SELECT SUM(total_amount - paid_amount) as total FROM people_ledger WHERE type='lent'").fetchone()
        lent_amount = row_lent['total'] if row_lent['total'] else 0.0

        row_borrowed = conn.execute("SELECT SUM(total_amount - paid_amount) as total FROM people_ledger WHERE type='borrowed'").fetchone()
