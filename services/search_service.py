...
class SearchService:
    ...
    def get_search_index():
        ...
        txs = conn.execute("""
            SELECT t.id, t.amount, t.category, t.date, t.type, a.name as account_name 
            FROM transactions t 
        ...
        # 3. People
        people = conn.execute("SELECT id, person_name, SUM(total_amount - paid_amount) as balance FROM people_ledger GROUP BY person_name").fetchall()
        index["people"] = [dict(p) for p in people]
        
        # 4. Loans
        loans = conn.execute("SELECT id, name, (total_to_pay - paid_amount) as remaining FROM loans WHERE status = 'active'").fetchall()
        index["loans"] = [dict(l) for l in loans]

        # 5. Categories
        cats = conn.execute("SELECT DISTINCT name, type FROM categories").fetchall()
